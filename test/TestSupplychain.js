...
contract('SupplyChain', function(accounts) {
    ...
    async function InitializeSupplyChain() {
        ...
        if(!(await consumerRole.isConsumer(consumerID))) {
            await consumerRole.addConsumer(consumerID);
        }
            
        if(!(await distributorRole.isDistributor(distributorID))) {
            await distributorRole.addDistributor(distributorID);
        }

        if(!(await retailerRole.isRetailer(retailerID))) {
            await retailerRole.addRetailer(retailerID);
        } 

        if(!(await farmerRole.isFarmer(originFarmerID))) {
